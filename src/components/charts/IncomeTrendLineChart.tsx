...
export function IncomeTrendLineChart({ entries }: Props) {
  if (entries.length === 0 || entries.every((e) => e.income === 0)) {
    return (
      <EmptyState icon="📊" title="No data yet" message="Keep logging income to see your income trend." />
    );
  }

  const data = entries.map((e) => ({
    value: fromMinorUnits(e.income),
    ...
    <View>
      <View style={styles.summary}>
        <View>
          ...
          <Text style={[styles.summaryValue, { color: PALETTE.income }]}>{formatCurrency(total)}</Text>
        </View>
      ...
      <View style={styles.chartWrap}>
        <LineChart
          ...
          initialSpacing={16}
          areaChart
          startFillColor={PALETTE.income}
          startOpacity={0.18}
          endFillColor={PALETTE.income}
          endOpacity={0.02}
        />
      </View>
    </View>
