      ...
      <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setVisible(false)}>
        <View style={styles.modal}>
          ...
          <FlatList
            ...
            contentContainerStyle={accountCategories.length === 0 ? styles.emptyContainer : styles.listContainer}
            ListEmptyComponent={
              !loading ? (
                <EmptyState
                  icon="🏦"
                  title="No account categories yet"
                  message="Add categories from the More tab to start organizing accounts."
                />
              ) : null
            }
            renderItem={({ item }) => (
